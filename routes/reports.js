...
router.get('/station/:stationId', isAuthenticated, async (req, res) => {
  try {
    const { stationId } = req.params;
    const reports = await getReportsByStation(stationId);

    res.render('reportsByStation', {
      title: `Reports for Station ${stationId}`,
      user: req.session.user,
      stationId,
      reports
    });
...
router.post('/:id/vote', isAuthenticated, async (req, res) => {
  try {
    const { type } = req.body;
    await voteReport(req.params.id, req.session.user.userId, type);

    const back = req.get('referer') || '/reports';
    res.redirect(back);
  } catch (e) {
    const back = req.get('referer') || '/reports';
    res.status(400).render('error', { error: e.toString(), user: req.session.user, back });
  }
});
